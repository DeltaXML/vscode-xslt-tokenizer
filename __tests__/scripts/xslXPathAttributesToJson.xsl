...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/" mode="#all">
    ...
    <xsl:variable name="sourceFileName" as="item()*" select="tokenize($testSourceURL, '/')[last()]"/>
    <xsl:variable name="result" as="map(*)">
      <xsl:map>
        ...
        <xsl:map-entry select="string(/*/@version)" key="'xsltVersion'"/>
        <xsl:map-entry select="'generated by: ' || static-base-uri()" key="'notes'"/>
